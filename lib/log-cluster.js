...
var LogCluster = exports.LogCluster = function(options, emitter){
    ...
    var server = options.app || http.createServer(function(request, response) {
            ...
            logger.log('info', '[log-cluster] received: ' + request.url);
            response.send('', 404);
        }),
    ...
    logger.log('info', 'listening:' + actualOptions.port);
    ...
    logCluster.listen(function(cb){
        ...
        (_.isFunction(actualOptions.LogListener) ? actualOptions.LogListener(wss, emitter) : actualOptions.LogListener).logger = logger;
        (_.isFunction(actualOptions.LogBuffer) ? actualOptions.LogBuffer(emitter) : actualOptions.LogBuffer).logger = logger;
        (_.isFunction(actualOptions.LogPublisher) ? actualOptions.LogPublisher(emitter) : actualOptions.LogPublisher).logger = logger;
