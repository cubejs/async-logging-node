...
var VALID_TYPES = ['atomicEvent', 'heartbeat', 'begin', 'end'];
...
var LogBuffer = exports.LogBuffer = function(emitter, mapper){
    ...
    emitter.on('log', function(log){
        ...
        assert.ok(_.contains(VALID_TYPES, clazz));//must be one of the valid event types
    ...
    emitter.on('clean', function(till){
        ...
        _.each(self.transactions, function(tx, req){
            if(tx.timestamp <= till){
                var transaction = self.transactions[req];
                if(!transaction.emitted && validateTree(transaction.family, transaction)){
                    var tree = makeTree(transaction.family, transaction);
