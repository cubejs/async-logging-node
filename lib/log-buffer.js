...
var makeTree = function(transaction, mapper){

    var log = transaction.log,
        root = transaction.root,
        family = transaction.family,
        children = transaction.family[root] || [],
        makeNode = function(family, child){
            if(!child){
                return null;
            }
            var mapped = mapper(child),
                log = mapped.log,
                type = mapped.type,
                children = family[mapped.event] || [];
            //closing of transaction ignored.
            if(_.isEqual("transaction", type) && mapped.end){
                return null;
            }
            ...
            return {
                log:log,
                type: type,
                children: _.compact(_.map(children, function(c){
                    return makeNode(family, c);
                }))
            };
        };
...
var LogBuffer = exports.LogBuffer = function(emitter, mapper){
    ...
    emitter.on("log", function(log){
        ...
        if(parent === "0" && !begin && !end){
            ...
            emitter.emit("transaction", makeTree(transaction, self.mapper));
        }
        else{
            ...
            var root = self.transactions[request].root,
                family = self.transactions[request].family;
